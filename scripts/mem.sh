...
print_mem() {
  ...
  local mem_free=$(echo $mem_usage | $_AWK -v scale="$size_scale" '{ print $1/scale }')
  local mem_used=$(echo $mem_usage | $_AWK -v scale="$size_scale" '{ print $2/scale }')
  ...
  mem_view="${mem_view//'#{mem.color}'/$(echo "$mem_color" | $_AWK '{ print $1 }')}"
  mem_view="${mem_view//'#{mem.color2}'/$(echo "$mem_color" | $_AWK '{ print $2 }')}"
  mem_view="${mem_view//'#{mem.color3}'/$(echo "$mem_color" | $_AWK '{ print $3 }')}"
...
get_mem_usage_osx(){
  ...
  vm_stat | $_AWK -v page_size=$page_size -F ':' '
    BEGIN { free=0; used=0 }
...
get_mem_usage_freebsd(){
  vmstat -H | tail -n 1 | $_AWK '{ print $5, $4 }'
}
